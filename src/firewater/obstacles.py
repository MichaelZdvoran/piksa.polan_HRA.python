...
class HazardPool:
    ...
    def __init__(self, x: float, y: float, width: float, height: float, hazard_type: str = "lava", color: tuple = None):
        self.rect = pygame.Rect(x, y, width, height)
    ...
    def draw(self, screen):
        if self.hazard_type == "lava":
            ...
            base_color = self.color if self.color else COLOR_LAVA
            ...
            color = self.color if self.color else COLOR_WATER_POOL
...
class Door:
    ...
    def __init__(self, x: float, y: float, player_type: PlayerType, locked: bool = True, color: tuple = None):
    ...
    def draw(self, screen):
        color = self.color if self.color else (COLOR_WATER if self.player_type == PlayerType.WATER else COLOR_FIRE)
